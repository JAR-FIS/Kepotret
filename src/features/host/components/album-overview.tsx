...
export function AlbumOverview({ albumId }: { albumId: string }) {
  ...
  const summary = { album_id: album.album_id, readiness: album.readiness, capture_state: album.capture_state, reveal_state: album.reveal_state, setup_revision: album.setup_revision, schedule_version: album.schedule_version };
  return <div className="space-y-5">
    <section className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-[var(--font-display)] text-xl font-bold">{t(`detail.${album.readiness === 'READY' ? 'ready' : 'draft'}`)}</h2><AlbumStatus album={summary} label={t(`albums.${albumStatusKey(summary)}`)} /></div>
